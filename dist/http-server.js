...
export class HttpServer {
    ...
    options;
    port = 0;
    constructor(reviewManager, mcpService, options) {
    ...
    setupRoutes() {
        ...
        this.app.get("/api/reviews/latest", async (req, res) => {
        ...
        // Update Comment
        this.app.put("/api/reviews/:id/comments/:commentId", async (req, res) => {
            try {
        ...
        // Delete Comment
        this.app.delete("/api/reviews/:id/comments/:commentId", async (req, res) => {
            try {
        ...
        this.app.put("/api/reviews/:id/plan", async (req, res) => {
            try {
        ...
        // Get Version List
        this.app.get("/api/reviews/:id/versions", async (req, res) => {
            try {
                const review = await this.reviewManager.getReview(req.params.id);
                if (!review) {
                    res.status(404).json({ error: "Review not found" });
        ...
        this.app.get("/api/reviews/:id/versions/:hash", async (req, res) => {
            try {
                const review = await this.reviewManager.getReview(req.params.id);
                if (!review) {
                    res.status(404).json({ error: "Review not found" });
        ...
        this.app.get("/api/reviews/:id/diff", async (req, res) => {
            try {
        ...
        this.app.post("/api/reviews/:id/rollback", async (req, res) => {
            try {
        ...
        // SSE Event Stream
        this.app.get("/api/reviews/:id/events", async (req, res) => {
            const review = await this.reviewManager.getReview(req.params.id);
            if (!review) {
                res.status(404).json({ error: "Review not found" });
                return;
        ...
        this.app.post("/api/reviews/:id/comments/:commentId/answer", async (req, res) => {
            try {
                ...
                const comment = await this.reviewManager.answerQuestion(req.params.id, req.params.commentId, answer);
                if (!comment) {
