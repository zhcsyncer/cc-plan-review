...
async function startHttpServer() {
  ...
  const httpOnlyPath = import_path.default.join(__dirname, "..", "http-only.cjs");
  debug("HTTP server path", { httpOnlyPath });
  return new Promise((resolve) => {
    const child = (0, import_child_process.spawn)("node", [httpOnlyPath], {
      detached: true,
...
function formatSuggestions(comments, planContent) {
