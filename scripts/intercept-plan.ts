...
interface Review {
  id: string;
  status: 'pending' | 'submitted_feedback' | 'questions_pending' | 'approved' | 'revised';
  comments: Array<{
    quote: string;
    comment: string;
    resolved?: boolean;
        ...
        for (const line of lines) {
          if (line.startsWith('data: ')) {
            try {
              ...
              if (event.type === 'status_changed' && event.status === 'approved') {
                resolved = true;
                req.destroy();
                // 获取最新的 review 数据
...
请告知用户：完成审核后，在终端输入 "continue"，然后调用 get_review_result 工具获取审核结果。`
    ...
    if (reviewResult.approvedDirectly || reviewResult.comments.filter(c => !c.resolved).length === 0) {
